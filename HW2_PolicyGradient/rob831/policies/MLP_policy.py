...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        if self.nn_baseline:
            assert q_values is not None, "q_values must be provided when using baseline"

            q_values = ptu.from_numpy(q_values)
            q_values_normalized = (q_values - q_values.mean()) / (q_values.std() + 1e-8)

            baseline_preds = self.baseline(observations).squeeze()

            baseline_loss = self.baseline_loss(baseline_preds, q_values_normalized)
