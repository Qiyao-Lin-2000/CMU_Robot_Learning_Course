...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while not done and steps < max_path_length:
        if render:
            frame = env.render(mode=render_mode[0])
            image_obs.append(frame)
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    timesteps_so_far = 0
    paths = []
    while timesteps_so_far < min_timesteps_per_batch:
        path = sample_trajectory(env, policy, max_path_length, render, render_mode)
        paths.append(path)
        timesteps_so_far += get_pathlength(path)
    return paths, timesteps_so_far
