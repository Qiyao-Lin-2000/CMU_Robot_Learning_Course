...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        next_ob_no_t = ptu.from_numpy(next_ob_no)
        reward_n_t = ptu.from_numpy(reward_n)
        terminal_n_t = ptu.from_numpy(terminal_n)

        total_loss = 0.0
        for _ in range(self.num_target_updates):
            with torch.no_grad():
                v_next = self.forward(next_ob_no_t)
                v_next = v_next * (1.0 - terminal_n_t)
                target_values = reward_n_t + self.gamma * v_next

            for _ in range(self.num_grad_steps_per_target_update):
                v_pred = self.forward(ob_no_t)
                loss = self.loss(v_pred, target_values)
